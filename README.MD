...
## Pré-requis:
- Une clé API est requise pour faire des recherches dans la base de données OpenAlex
  ([*How to get an EPO OPS API key*](https://help.openalex.org/api/authentication/)),
  la clé doit être insérée dans la fonction
  *search_openalex.def config_openalex()*.
...
## Utilisation:
- Appel du script: *pyrefsearch\pyrefsearch.py data\\<fichier_toml\>*, où *\<fichier_toml\>* est 
par exemple *pyrefsearch.toml* ou *pyrefsearch_last_month.toml*
- Les paramètres d'exécution sont lus dans le fichier *\<fichier_toml\>*
- Les données d'auteur.e.s sont lues dans un fichier Excel d'entrée dont le nom du fichier est spécifié dans *\<fichier_toml\>*
- Les résultats de la recherche sont écrits dans un fichier Excel de sortie
- Tous les fichiers spécifiés sont lus/écrits dans le même répertoire que le fichier *\<fichier_toml\>*
...
- *member_status* : "Régulier" (valeur par défaut) ou "Collaborateur"
- *date_start*, *date_end* : Première/dernière dates de la période de recherche
- *publication_types_openalex* : List des binômes de types de publications et mots-clefs (OpenAlex "subtype") correspondants
  pour la recherche de publications
- *local_affiliations* : Liste des institutions considérées comme des affiliations "locales"
- *espacenet_patent_search* : activer la recherche de brevets *espacenet* ("espacenet_patent_search = true")
- *espacenet_patent_search_results_file* = nom de fichier de résultats de recherche précédante dans *espacenet* (si ce paramètre
...
Pour forcer une recherche dans le mois précédant, en plus des paramètres ci-haut (voir *pyrefsearch.toml*) où
*date_start*, *date_end* sont ignorés, les paramètres suivants sont requis:
- *previous_month_publications_search* = true
- *previous_month_publications_search_confirmation_emails* : liste des adresses qui recevront un courriel avec le fichier Excel de résultats

## Fichier Excel d'entrée "*in_excel_file*" (voir le fichier *\<fichier_toml\>*), feuillet *Membres* :
...
## Fichier Excel de sortie "*\<in_excel_file mantissa\>\_publications\_\<date_start\>-\<date_end\>.xlsx*" :
  ...
  *Brevets INPADOC (délivrés)* : résultats de la
  ...
  publications & brevets:
    - *Affl.* : une institution ou une affiliation est comprise dans la liste *local_affiliations*
